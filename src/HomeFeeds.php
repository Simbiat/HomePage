    ...
    private function feed(array $uri, string $format = 'atom'): array
    {
        ...
        if (!empty($uri[0])) {
            ...
            if (!empty($title)) {
                ...
                if ($format === 'atom') {
                    $query = match($uri[0]) {
                        'bicchanged' => 'SELECT CONCAT(\'https://'.$_SERVER['HTTP_HOST'].'/bictracker/bic/\', `VKEY`) as `link`, `NAMEP` as `title`, `DT_IZM` as `updated`, \'Central Bank of Russia\' AS `author_name`, \'https://cbr.ru/\' AS `author_uri`, `NAMEP` as `summary`, `DT_IZM` as `published`, \'Central Bank of Russia\' AS `source_title`, \'https://cbr.ru/\' AS `source_id`, `DT_IZM` as `source_updated` FROM `bic__list` a WHERE `DATEDEL` IS NULL ORDER BY `DT_IZM` DESC LIMIT 25',
                        'bicdeleted' => 'SELECT CONCAT(\'https://'.$_SERVER['HTTP_HOST'].'/bictracker/bic/\', `VKEY`) as `link`, `NAMEP` as `title`, `DT_IZM` as `updated`, \'Central Bank of Russia\' AS `author_name`, \'https://cbr.ru/\' AS `author_uri`, `NAMEP` as `summary`, `DT_IZM` as `published`, \'Central Bank of Russia\' AS `source_title`, \'https://cbr.ru/\' AS `source_id`, `DT_IZM` as `source_updated` FROM `bic__list` a WHERE `DATEDEL` IS NOT NULL ORDER BY `DATEDEL` DESC LIMIT 25',
                    };
                } elseif ($format === 'rss') {
                    $query = match($uri[0]) {
